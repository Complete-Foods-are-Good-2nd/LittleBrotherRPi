...
Communication::Communication()
{
    ...
    {
        std::cerr << "Error: The environment variable: LB_SERIAL_DEVICE_DIR is not set." << std::endl;
        exit(1);
    }
...
std::pair<std::string, std::string> Communication::generate_loopback_serial_devices()
{
    ...
    for (const auto serial_device_name : after_serial_device_names)
    {
        if (!before_serial_device_names.count(serial_device_name))
        {
            ...
            {
                std::cerr << "Error: Too many virtual-serial-devices, not 2." << std::endl;
                exit(1);
            }
    ...
    {
        std::cerr << "Error: Too few virtual-serial-devices, not 2." << std::endl;
        exit(1);
    }
...
std::set<std::string> Communication::get_serial_device_names()
{
    ...
    if (dir_stream == NULL)
    {
        std::cerr << "Error: Can't open dir: " << serial_device_dir << std::endl;
        exit(1);
    }
...
void Communication::start_stream()
{
    ...
    {
        std::cerr << "Error: The environment variable: LB_AYAME_SIGNALINGKEY is not set." << std::endl;
        exit(1);
    }
    ...
    {
        std::cerr << "Error: The environment variable: LB_AYAME_ROOM_ID is not set." << std::endl;
        exit(1);
    }
    ...
    {
        std::cerr << "Error: The environment variable: LB_MOMO_DIR is not set." << std::endl;
        exit(1);
    }
